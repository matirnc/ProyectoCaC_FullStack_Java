...
loginForm.addEventListener('submit', (e)=>{
    ...
    if(!valid_user){
        ...
        if (window.confirm(`Bienvenido ${valid_user.nombre}. `)) {
            ...
            document.getElementById('login_registro').textContent = valid_user.nombre;
        ...
        /*
        alert(`Bienvenido ${valid_user.nombre}`);

        document.querySelector('#login_registro').style.display = 'none';
        document.querySelector('#mi_perfil').style.display = 'block';

        
        //window.location.href = '../index.html';

        // Cambiar el texto del enlace NO ESTA FUNCIONANDO
        /*
        const usuario_logueado = document.getElementById('usuario_logueado');
        if (usuario_logueado) {
            usuario_logueado.textContent = `Bienvenido, ${valid_user.nombre}`;
        }
        */
